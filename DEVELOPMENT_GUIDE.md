...
**Local dev CORS:** The SPA (e.g. `http://localhost:3000`) calls the API directly. Ensure the backend allows that origin (and `http://127.0.0.1:3000` if you use it) and returns **200** on **OPTIONS** preflight. See `docs/shared/adrs/frontend/adr-005-miniquiz-completion-cors.md` (worksheet CDN CORS is separate).
